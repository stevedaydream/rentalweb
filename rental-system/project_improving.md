...
### 退租點交已接上雙方流程（2026-08-23 完成）
- ✅ 租客抽屜新增「退租點交」，開啟 `InspectionSession`（`type: 'moveout'`）
- ✅ 品項以該租客**已簽署的入住點交**為基準帶入，每項附 `baseline`（入住當時的
  狀況、說明與照片），租客與房東在現場直接對照
- ✅ `MoveOutWizard` 改讀已簽署的退租點交：狀況欄鎖定顯示（那是雙方簽過名的結論，
  不該在結算畫面單方改掉），賠償比例以 `suggestedRatio` 依惡化程度預填後仍可調整
- ✅ 沒做雙方點交時保留舊的手動路徑並明確警示舉證力較弱——租客已經走了也要能結案
- ✅ 退租執行時 `stampPhotoCleanup` 標記點交原檔的可刪除時點
