<template>
  <div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="p-6 bg-white dark:bg-card-dark rounded-2xl border border-ink-100 dark:border-ink-800 shadow-sm flex items-center justify-between">
        <div>
        ...
        <div class="w-12 h-12 rounded-full bg-gold-50 dark:bg-gold-900/20 flex items-center justify-center text-gold-600">
    ...
    <div class="bg-white dark:bg-card-dark rounded-2xl border border-ink-100 dark:border-ink-800 shadow-sm overflow-hidden">
      <div class="p-4 border-b border-ink-100 dark:border-ink-800 flex flex-col md:flex-row gap-4 justify-between items-center">
      ...
      <div class="relative bg-white dark:bg-card-dark rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div class="p-6 border-t border-ink-100 dark:border-ink-700 flex justify-end gap-3">
          ...
          <button 
            @click="saveItem"
            :disabled="submitting"
            class="px-5 py-2 rounded-xl bg-gold-500 text-white font-bold shadow-md shadow-gold-500/20 hover:bg-gold-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
          >
...
const list = ref<AnnouncementDoc[]>([])
...
onMounted(() => {
  ...
  unsubscribe = subscribeAnnouncements(
    authStore.effectiveUid,
    (items) => { list.value = items; loading.value = false },
    (err) => { console.error('讀取公告失敗:', err); loading.value = false }
  )
})
...
const filteredList = computed(() => {
  ...
  if (currentFilter.value === 'pinned') result = result.filter(i => i.isPinned || i.category === '重要公告')
  else if (currentFilter.value === 'normal') result = result.filter(i => i.category === '一般通知')
...
// --- Actions ---
const openModal = (item?: AnnouncementDoc) => {
  if (item) {
    isEditing.value = true
    editingId.value = item.id
    form.value = { title: item.title, category: item.category, content: item.content, isPinned: item.isPinned }
  } else {
    isEditing.value = false
    editingId.value = null
    form.value = { title: '', category: '一般通知', content: '', isPinned: false }
  }
  showModal.value = true
}

const saveItem = async () => {
  if (!form.value.title || !form.value.content) { toast.warning('請填寫完整內容'); return }
  ...
  try {
    if (isEditing.value && editingId.value) {
      ...
      await addAnnouncement(authStore.effectiveUid, form.value)
    }
    showModal.value = false
    toast.success(isEditing.value ? '公告已更新' : '公告已發布')
  } catch (e: any) {
...
const togglePin = async (item: AnnouncementDoc) => {
  try {
    await toggleAnnouncementPin(item.id, !item.isPinned)
  } catch (e) {
    console.error('切換置頂失敗:', e)
  }
}
</script>
