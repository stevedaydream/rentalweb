...
function setTenantSignature(img) {
  ...
  showTenantSignModal.value = false
}
...
function payerText(val) {
  ...
  return val // backward compat: already a display string
}

const feeWaterText = computed(() => payerText(props.form?.feeWater ?? 'landlord'))
const feeGasText = computed(() => payerText(props.form?.feeGas ?? 'none'))
const feeInternetText = computed(() => payerText(props.form?.feeInternet ?? 'landlord'))
const feeManagementText = computed(() => payerText(props.form?.feeManagement ?? 'none'))
const feeElectricityText = computed(() => {
  const base = payerText(props.form?.feeElectricity ?? 'tenant')
  const note = props.form?.feeElectricityNote
...
.section-title { color: #2767c7; font-weight: bold; margin-top: 22px; }
.signature-table {
  width: 100%; margin-top: 30px; margin-bottom: 12px;
  border-collapse: separate; border-spacing: 0 14px;
}
.signature-table th {
  text-align: center; font-size: 1.09em; color: #2767c7; font-weight: 600;
  padding-bottom: 6px; letter-spacing: 1.5px; border: none; background: none;
}
.signature-table td { text-align: center; border: none; background: none; }
.signature-cell {
  display: flex; flex-direction: column; align-items: center;
  justify-content: flex-end; min-height: 90px; padding: 8px 0 0 0;
}
.signature-img {
  height: 60px; max-width: 120px; border: 1px solid #aaa;
  margin-bottom: 6px; background: #fafafa; object-fit: contain; cursor: pointer;
}
.sign-line { width: 100px; height: 2px; background: #ccc; margin-top: 6px; border-radius: 2px; }
.sign-hint { color: #888; font-size: 0.88em; margin-top: 4px; }
</style>
...
<template>
  <div class="preview-container">
    ...
    <ul>
      <li>管理費：{{ feeManagementText }}</li>
      <li>水費：{{ feeWaterText }}</li>
      <li>電費：{{ feeElectricityText }}</li>
      <li>瓦斯費：{{ feeGasText }}</li>
      <li>網路費：{{ feeInternetText }}</li>
    </ul>
    ...
    <ol>
      ...
      <li>承租人應經出租人同意始得將本租賃住宅之全部或一部分轉租、出借或以其他方式供他人使用，或將租賃權轉讓於他人。</li>
    ...
    <ol>
      ...
      <li>出租人與承租人簽訂本契約前，租賃住宅有由承租人負責修繕之項目及範圍者，出租人應先向承租人說明並經承租人確認，未經約明確認者，出租人應負責修繕。</li>
    </ol>
    ...
    <ol>
      ...
      <li>承租人違反前項義務，致租賃住宅毀損或滅失者，應負損害賠償責任。</li>
      <li>前項承租人應賠償之金額，得由第四條第一項規定之押金中抵充，如有不足，並得向承租人請求給付不足之金額。</li>
      <li>承租人經出租人同意轉租者，應於簽訂轉租契約後三十日內，以書面將相關資料通知出租人。</li>
    </ol>
    ...
    <ol>
      ...
      <li>承租人未依第一項規定返還租賃住宅時，出租人得向承租人請求未返還租賃住宅期間之相當月租金額及違約金至返還為止。</li>
      <li>前項金額出租人得由押金中抵充，如有不足，並得向承租人請求給付不足之金額或費用。</li>
    </ol>
    ...
    <p>租賃期間有下列情形之一者，出租人得提前終止租約：</p>
    <ol>
      <li>出租人為重新建築而必要收回。</li>
      <li>承租人遲付租金之總額達二個月之租金額，經催告仍不為支付。</li>
      <li>承租人違反第七條第一項規定，擅自變更用途。</li>
      <li>承租人擅自轉租或轉讓租賃權。</li>
      <li>承租人毀損租賃住宅或附屬設備，經催告仍不修繕或賠償。</li>
      <li>承租人未經同意擅自進行室內裝修。</li>
    </ol>

    <div class="section-title">第十七條　承租人提前終止租約</div>
    <p>租賃期間有下列情形之一，致難以繼續居住者，承租人得提前終止租約：</p>
    <ol>
      <li>租賃住宅未合於所約定居住使用，並有修繕之必要，經催告仍不修繕。</li>
      <li>租賃住宅因不可歸責承租人之事由致一部滅失，且其存餘部分不能達租賃之目的。</li>
      <li>租賃住宅有危及承租人或其同居人之安全或健康之瑕疵。</li>
    ...
    <p>
      租賃關係消滅，完成點交之手續後，承租人仍於租賃住宅有遺留物者，經出租人定相當期限催告，屆期仍不取回時，視為拋棄其所有權。<br>
      出租人處理前項遺留物所生費用，得由押金中抵充。
    </p>

    <div class="section-title">第十九條　履行本契約之通知</div>
    <p>
      租賃雙方相互間之通知，以郵寄為之者，應以本契約所記載之地址為準。<br>
    ...
    <p>
      ...
      <template v-if="form.customArticle21">
        <br>{{ form.customArticle21 }}
      </template>
    </p>
          ...
          <tr>
            ...
            <td>
              <div class="signature-cell">
                <img
                  v-if="form.landlordSignature"
                  :src="form.landlordSignature"
                  class="signature-img"
                  alt="出租人簽名"
                  @click="showLandlordSignModal = true"
                />
                <div v-else
                  class="signature-img flex items-center justify-center text-gray-400 text-xs cursor-pointer"
                  @click="showLandlordSignModal = true"
                >點擊簽名</div>
                <div class="sign-line"></div>
                <div class="sign-hint">點擊以簽名</div>
              </div>
            </td>
            <!-- 承租人 -->
            <td>
              <div class="signature-cell">
                <img
                  ...
                  class="signature-img"
                  alt="承租人簽名"
                  @click="showTenantSignModal = true"
                />
                <div v-else
                  class="signature-img flex items-center justify-center text-gray-400 text-xs cursor-pointer"
                  @click="showTenantSignModal = true"
  ...
  <Signature v-model:visible="showTenantSignModal" @confirm="setTenantSignature" />
