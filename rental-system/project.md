...
- 批量建立：`components/rooms/BatchRoomWizard.vue`（建物分頁「新增建物」兩步驟、卡片「批量新增房間」、房源分頁「批量新增」）。樓層範圍＋每層間數（可逐層調整）＋房號規則（樓層＋兩位序號／樓層＋字母，前後綴）產生預覽，可逐列改房號、租金、坪數、格局或刪列；缺的樓層子群組以「4F」建立（既有「4樓」視為同層）。房間建為待租、不公開、無照片，寫入前重新讀取房號再比對。規則在 `utils/roomBatch.ts`（11 項測試）。
- 同一房東房號必須唯一（`roomNameKey` 忽略空白、大小寫、全形）：系統多處以房號文字對應房間，重名會安靜地對錯房。批量與單間存檔都檢查；改用房間 id 全面對應列為技術債。
...
- ~~租客可自行改寫 `users.landlordId`，等於能挑房東看 properties／meter_readings~~（2026-09-04 已解決：綁定改走 callable `bindLandlordByCode`／`unbindLandlord`，伺服端查驗邀請碼後代寫，規則同步鎖住租客自改 `landlordId`）
...
- 預收餘額（`tenants.credit`）在沖抵到帳單之前不計入月度／年度的「已收」：收入仍以帳單為單位，預繳的錢要等下期帳單生成、沖抵後才入帳
- LINE「帳單提醒」批次模式只通知本月有待收帳單的租客（訊息會附前期未繳）；只有前期欠款、本月沒帳單的租客不會收到，需用單一租客提醒
...
| 2026-08-23 | **租客可見性與列印圖片修復**：①**租客端「我的合約」永遠空白** —— `signed_contracts.tenantUid` 兩條建立路徑（獨立簽約頁 `newPrefill`、上線精靈 `contractPrefill`）都沒帶租客 UID，`ContractForm` 的租客下拉也只抄姓名／證號／電話，於是每份電子合約都存成 `tenantUid: null`，而租客端以 `where(tenantUid == uid)` 查詢、Firestore 規則的電子確認也要求 `tenantUid` 相符（紙本上傳路徑有帶，故只有紙本看得到）。三處補上 UID；既有舊約由房東端「合約→歷史」載入時依證件號碼比對租客檔案自動補寫（`backfillTenantUid`，房東對自己的合約有寫入權，租客端無法自救）。②**平板列印點交單看不到瑕疵存證照片** —— `printHtmlPdf` 的就緒判斷只等 `load` 與字型，安全網 4 秒到就強制送印，Storage 上的遠端照片來不及進版面。改為送印前先把跨來源圖片抓成 data URL（抓不到維持原網址）、再逐張等 `img.complete`，安全網放寬到 15 秒。③**帳單詳情下載圖片不含匯款截圖** —— 原本以 `data-capture-skip` 主動排除（跨來源圖內嵌失敗會讓整張擷取失敗）；改為 `captureElementPng` 內部先行內嵌，單張失敗才就地標記排除，截圖因此正常入鏡。④租客報修 Modal 在平板下半部被裁 —— Modal 與底部 Tab Bar 同為 `z-50` 且在其之後繪製，改 `Teleport` 到 body、`z-[100]`，並改成 header／可捲動內容／固定底部按鈕的三段式（`max-h-[92dvh]`）。⑤房東端未讀訊息提醒改為 FB 式紅色角標（圖示右上角數字），手機版漢堡鈕同步顯示未讀數 |
| 2026-08-23 | **大樓資訊重做**：①**租客端根本看不到房東存的圖** —— 租客端是照舊結構寫的，`mapImageUrl` 讀成 `customImageUrl`（自訂底圖永遠不顯示）、用 `mapType` 當 key 去查模板（一律 fallback 成空白圖）、設施讀 `description` 而房東存的是 `location`／`rules`（規範只剩名稱）；兩邊的模板 SVG 還是各畫各的（480×300 vs 800×500，房間配置不同），標記位置對不上房間。抽出 `utils/buildingMap.ts` 共用型別／標記設定／模板與 `normalizeBuildingInfo()`（吸收舊欄位名），兩端一律走同一份。②**一棟一張圖**：`buildingInfo` 改存 `properties/{id}`，房東端加建物切換器，租客端經「租客→房號→rooms.propertyId」找自己那棟；尚未建立建物或該棟未設定時退回 `users/{uid}.buildingInfo`，舊資料在首次進入該棟時以草稿帶入、儲存才落地。`firestore.rules` 的 properties 補上租客讀取（比照 meter_readings 以 `users.landlordId` 限範圍）。③**放標記改成點選即放**：解鎖後左鍵點地圖就地跳出圖示選單（六類＋家電子選單），選了就放上去、名稱自動帶入（滅火器→「滅火器」、洗衣機→「洗衣機」），不再先填表單；備註改為之後點該標記再補。④標記可**拖曳**（pointer events，平板可用）與**方向鍵微調**（Shift 為 4 倍步進），新增**鎖定按鈕**（預設鎖定）避免瀏覽時誤觸。⑤繪圖工具收進「進階」摺疊區；「設施規範」分頁移除，改由「有備註的標記」在租客端自動彙整成設施說明，舊資料提供一鍵轉為地圖標記 |
